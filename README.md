...
**Folder Overview**
- **HTML**: Core HTML examples — [HTML/attributes.html](HTML/attributes.html), [HTML/Forms.html](HTML/Forms.html), [HTML/Form table.html](HTML/Form%20table.html), [HTML/Hyperlink.html](HTML/Hyperlink.html), [HTML/index.html](HTML/index.html), [HTML/Internalhyperlink.html](HTML/Internalhyperlink.html), [HTML/list.html](HTML/list.html), [HTML/media.html](HTML/media.html), [HTML/table creation.html](HTML/table%20creation.html)
...
- **Forms**: `action`, `method`, `name`, `type`, `value`, `placeholder`, `required`, `minlength`, `maxlength`, `checked`, `selected`, `pattern` (client-side validation).
- **Input Types**: `text`, `email`, `password`, `date`, `time`, `datetime-local`, `month`, `week`, `tel`, `url`, `search`, `number`, `range`, `color`, `file`, `radio`, `checkbox`.
- **Lists**: Ordered lists (`ol`), unordered lists (`ul`), definition lists (`dl`).
- **IDs and Classes**: `id`, `class` for styling and JS hooks.
- **Table Attributes**: `colspan`, `rowspan`, table headers and cells.
- **Meta Viewport**: `meta viewport` for responsiveness.
- **Missing / Recommended HTML topics**: semantic elements (`header`, `nav`, `main`, `section`, `article`, `footer`), ARIA roles and accessibility notes.
...
- **Text & Fonts**: `font-family`, `font-size`, `font-weight`, `color`, `text-align`, `line-height`, `text-decoration`, `text-shadow`, `text-indent`, `text-transform`.
- **Shapes & Visuals**: `border-radius`, `transform` (scale, skew, rotate, rotateX, rotateY), `background-image`, `background-size`, `filter` (brightness, drop-shadow, contrast, saturate, opacity).
- **Layout**: CSS Flexbox (`display: flex`, `justify-content`, `align-items`, `flex-wrap`), CSS Grid (`display: grid`, `grid-template-columns`, `gap`).
- **Transitions**: `transition`.
- **Missing / Recommended CSS topics**: media queries for responsive design, CSS variables (`--var`), animations (`@keyframes`), and advanced selectors.
